...
class ConsoleErrorTracker {
  ...
  getApplicationErrors(): ConsoleMessage[] {
    ...
    return this.errors.filter(err => {
      ...
      // Filter out AxiosError which occurs when API is unavailable in test environment
      // This is expected since we're testing the frontend in isolation without a backend
      const isAxiosNetworkError = err.text.includes("AxiosError") ||
                                  err.text.includes("Network Error")

      return !isNetworkError && !isAxiosNetworkError
    })
  }
